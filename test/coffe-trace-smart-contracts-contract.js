...
const { CoffeTraceSmartContractsContract } = require('..');
...
describe('CoffeTraceSmartContractsContract', () => {
    ...
    beforeEach(() => {
        contract = new CoffeTraceSmartContractsContract();
        ctx = new TestContext();
    ...
    describe('#coffeTraceSmartContractsExists', () => {

        it('should return true for a private asset that exists', async () => {
            await contract.coffeTraceSmartContractsExists(ctx, '001').should.eventually.be.true;
        });

        it('should return false for a private asset that does not exist', async () => {
            await contract.coffeTraceSmartContractsExists(ctx, '002').should.eventually.be.false;
        });

    });

    describe('#createCoffeTraceSmartContracts', () => {

        it('should throw an error for a private asset that already exists', async () => {
            await contract.createCoffeTraceSmartContracts(ctx, '001').should.be.rejectedWith('The asset coffe trace smart contracts 001 already exists');
        });
        ...
        it('should throw an error if transient data is not provided when creating private asset', async () => {
            ...
            await contract.createCoffeTraceSmartContracts(ctx, '002').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should create a private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.createCoffeTraceSmartContracts(ctx, '002');
            ctx.stub.putPrivateData.should.have.been.calledOnceWithExactly(collectionName, '002', Buffer.from('{"privateValue":"1500"}'));
        });

    });

    describe('#readCoffeTraceSmartContracts', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.readCoffeTraceSmartContracts(ctx, '003').should.be.rejectedWith('The asset coffe trace smart contracts 003 does not exist');
        });

        it('should return my private asset', async () => {
            await contract.readCoffeTraceSmartContracts(ctx, '001').should.eventually.deep.equal({ privateValue: '150' });
            ctx.stub.getPrivateData.should.have.been.calledWithExactly(collectionName, '001');
        });

    });

    describe('#updateCoffeTraceSmartContracts', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.updateCoffeTraceSmartContracts(ctx, '003').should.be.rejectedWith('The asset coffe trace smart contracts 003 does not exist');
        });

        it('should throw an error if transient data is not provided when updating private asset', async () => {
            let transientMap = new Map();
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateCoffeTraceSmartContracts(ctx, '001').should.be.rejectedWith('The privateValue key was not specified in transient data. Please try again.');
        });
        ...
        it('should update my private asset if transient data key is privateValue', async () => {
            ...
            ctx.stub.getTransient.returns(transientMap);
            await contract.updateCoffeTraceSmartContracts(ctx, '001');
    ...
    describe('#deleteCoffeTraceSmartContracts', () => {

        it('should throw an error for my private asset that does not exist', async () => {
            await contract.deleteCoffeTraceSmartContracts(ctx, '003').should.be.rejectedWith('The asset coffe trace smart contracts 003 does not exist');
        });

        it('should delete my private asset', async () => {
            await contract.deleteCoffeTraceSmartContracts(ctx, '001');
            ctx.stub.deletePrivateData.should.have.been.calledOnceWithExactly(collectionName, '001');
        });

    });

    describe('#verifyCoffeTraceSmartContracts', () => {
        ...
        it('should return success message if hash provided matches the hash of the private data', async () => {
            ...
            ctx.stub.getPrivateDataHash.withArgs(collectionName, '001').resolves(Buffer.from(hashToVerify, 'hex'));
            const result = await contract.verifyCoffeTraceSmartContracts(ctx, mspid, '001', '{"privateValue":"125"}');
        ...
        it('should throw an error if hash provided does not match the hash of the private data', async () => {
            ...
            const result = await contract.verifyCoffeTraceSmartContracts(ctx, mspid, '001', 'someObject');
        ...
        it('should throw an error when user tries to verify an asset that doesnt exist', async () => {
            ...
            await contract.verifyCoffeTraceSmartContracts(ctx, mspid, '001', 'someObject').should.be.rejectedWith('No private data hash with the key: 001');
        });
    });
});
